...
public class SolutionSet2 implements BranchBound {
    ...
    public SolutionSet2(Graph G, int start, Collection<Edge> skipped) { // start is articulation or skipped is already populated.
        ...
        this.adjacent = new TreeSet<>();
        for (Edge e : G.incident[start]) {
            this.adjacent.add(e.standard());
        }
    ...
    public List<BranchBound> branch() {
        ...
        if (isValid()) {
            result.add(new Solution2(G, edges, vertices));
        }

        List<Edge> sorted = new ArrayList<>(adjacent);
        sorted.sort(G::selectionOrder);
